...
export default function RelatorioPessoalPage() {
  const [data, setData] = useState<RelatorioData | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch("/api/admin/financeiro-pessoal/relatorio")
      .then((r) => r.json())
    ...
    <>
      <style>{`
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .header { display: flex; align-items: center; justify-content: space-between; padding: 22px 32px 18px; border-bottom: 3px solid #7c3aed; }
        .header img { height: 52px; width: auto; object-fit: contain; }
        .header-right { text-align: right; }
        .header-right h1 { font-size: 17px; font-weight: 700; color: #7c3aed; }
        .header-right p { font-size: 11px; color: #6b7280; margin-top: 2px; }

        .title-bar { background: #7c3aed; color: white; padding: 10px 32px; display: flex; justify-content: space-between; align-items: center; }
        .title-bar .title { font-size: 13px; font-weight: 600; }
        .title-bar .date  { font-size: 11px; opacity: 0.7; }

        .content { padding: 24px 32px; }
        .section { margin-bottom: 28px; }
        .section-title { font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.06em; color: #7c3aed; margin-bottom: 12px; padding-bottom: 5px; border-bottom: 1px solid #ede9fe; }
        ...
        .card-green  { border-color: #f2dbb6; } .card-green  .value { color: #8b682b; }
        ...
        tfoot td { padding: 8px 10px; font-weight: 700; border-top: 2px solid #d1d5db; background: #faf5ff; }
        tfoot td.right { text-align: right; }

        .badge { display: inline-block; padding: 2px 7px; border-radius: 99px; font-size: 9px; font-weight: 700; }
        .badge-green  { background: #f9edd8; color: #70521d; }
        ...
        .footer { margin-top: 32px; padding: 14px 32px; border-top: 1px solid #ede9fe; display: flex; justify-content: space-between; font-size: 10px; color: #9ca3af; }

        .btn-print { position: fixed; bottom: 24px; right: 24px; background: #7c3aed; color: white; border: none; padding: 12px 20px; border-radius: 10px; cursor: pointer; font-size: 13px; font-weight: 600; box-shadow: 0 4px 12px rgba(124,58,237,0.3); z-index: 999; display: flex; align-items: center; gap: 8px; }
      ...
      <div className="title-bar">
        <span className="title">Controle Financeiro Pessoal</span>
      ...
      <div className="content">
        ...
        <div className="summary-grid">
          ...
          <div className={`summary-card ${resumo.saldo >= 0 ? "card-purple" : "card-orange"}`}>
        ...
        {porCategoria.receitas.length > 0 && (
          <div className="section">
            ...
            <table>
              <thead>
                <tr>
                  <th>Categoria</th><th className="right">Qtd</th><th className="right">Total</th><th className="right">%</th><th style={{ width: "140px" }}>Participação</th>
                </tr>
              </thead>
              <tbody>
                {porCategoria.receitas.map((c) => (
                  <tr key={c.categoria}>
                    ...
                    <td className="right" style={{ color: "#8b682b", fontWeight: 600 }}>{fmt(c.total)}</td>
                    <td className="right">{pct(c.total, resumo.totalReceitas)}</td>
                    <td><div className="bar-wrap"><div className="bar-bg"><div className="bar-fill" style={{ width: pct(c.total, resumo.totalReceitas), background: "#c49e62" }} /></div></div></td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr><td>Total</td><td className="right">{porCategoria.receitas.reduce((a, c) => a + c.count, 0)}</td><td className="right" style={{ color: "#8b682b" }}>{fmt(resumo.totalReceitas)}</td><td className="right">100%</td><td /></tr>
              </tfoot>
            </table>
          </div>
        )}
              ...
              <tbody>
                {porCategoria.despesas.map((c) => (
                  <tr key={c.categoria}>
                    ...
                    <td><div className="bar-wrap"><div className="bar-bg"><div className="bar-fill" style={{ width: pct(c.total, resumo.totalDespesas), background: "#ef4444" }} /></div></div></td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr><td>Total</td><td className="right">{porCategoria.despesas.reduce((a, c) => a + c.count, 0)}</td><td className="right" style={{ color: "#dc2626" }}>{fmt(resumo.totalDespesas)}</td><td className="right">100%</td><td /></tr>
              </tfoot>
            </table>
          </div>
        )}
        ...
        {porFormaPagamento.length > 0 && (
          <div className="section">
            ...
            <table>
              <thead>
                <tr><th>Forma</th><th className="right">Qtd</th><th className="right">Total</th><th className="right">%</th><th style={{ width: "140px" }}>Participação</th></tr>
              </thead>
              <tbody>
                {porFormaPagamento.map((p) => {
                  ...
                  return (
                    <tr key={p.forma}>
                      <td style={{ fontWeight: 500 }}>{PAGAMENTO_EMOJI[p.forma] || ""} {p.label}</td>
                      ...
                      <td><div className="bar-wrap"><div className="bar-bg"><div className="bar-fill" style={{ width: pct(p.total, totalGeral), background: "#7c3aed" }} /></div></div></td>
                    </tr>
                  );
                })}
              </tbody>
        ...
        {porMes.length > 0 && (
          <div className="section">
            ...
            <table>
              <thead>
                <tr><th>Mês</th><th className="right">Receitas</th><th className="right">Despesas</th><th className="right">Saldo do Mês</th></tr>
              </thead>
              <tbody>
                {porMes.map((m) => (
                  <tr key={m.label}>
                    <td style={{ fontWeight: 500, textTransform: "capitalize" }}>{m.label}</td>
                    <td className="right" style={{ color: "#8b682b", fontWeight: 500 }}>{m.receitas > 0 ? fmt(m.receitas) : "—"}</td>
                    <td className="right" style={{ color: "#dc2626", fontWeight: 500 }}>{m.despesas > 0 ? fmt(m.despesas) : "—"}</td>
                    <td className="right"><span style={{ fontWeight: 700, color: m.saldo >= 0 ? "#7c3aed" : "#ea580c" }}>{fmt(m.saldo)}</span></td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr>
                  <td>Total Geral</td>
                  <td className="right" style={{ color: "#8b682b" }}>{fmt(resumo.totalReceitas)}</td>
                  <td className="right" style={{ color: "#dc2626" }}>{fmt(resumo.totalDespesas)}</td>
                  <td className="right" style={{ color: resumo.saldo >= 0 ? "#7c3aed" : "#ea580c" }}>{fmt(resumo.saldo)}</td>
                </tr>
              </tfoot>
            </table>
          </div>
        ...
        {lancamentos.length > 0 && (
          <div className="section page-break">
            ...
            <table>
              <thead>
                <tr>
                  <th>Data</th><th>Descrição</th><th>Categoria</th><th>Tipo</th><th>Pagamento</th><th className="right">Valor</th>
                </tr>
              </thead>
              <tbody>
                {lancamentos.map((l) => (
                  <tr key={l.id}>
                    ...
                    <td className="right" style={{ fontWeight: 700, color: l.tipo === "receita" ? "#8b682b" : "#dc2626", whiteSpace: "nowrap" }}>
