...
// ─── Categorias ─────────────────────────────────────────────────────────────
const CATEGORIAS_RECEITA = ["Comissão", "Honorários", "Consultoria", "Serviços", "Outros"];
const CATEGORIAS_DESPESA = [
  "Salários", "Aluguel", "Marketing", "Fornecedores",
  "Impostos", "Softwares", "Transporte", "Outros",
];
...
export default function FinanceiroPage() {
  ...
  async function salvar() {
    ...
    setSalvando(true);
    try {
      const url = editando ? `/api/admin/financeiro/${editando.id}` : "/api/admin/financeiro";
  ...
  async function excluir(id: string) {
    try {
      const res = await fetch(`/api/admin/financeiro/${id}`, { method: "DELETE" });
      const json = await res.json();
      if (!json.success) throw new Error(json.error);
  ...
  return (
    <div className="space-y-6">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div>
          <h1 className="text-3xl md:text-[38px] font-extrabold tracking-[-0.035em] text-zinc-950 dark:text-white">Controle Financeiro</h1>
      ...
      {resumo && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}
          ...
          {[
            ...
            { label: "Saldo",                 value: fmt(resumo.saldo),         icon: Wallet,       color: resumo.saldo >= 0 ? "bg-blue-50 text-blue-600 dark:bg-blue-900/20 dark:text-blue-400" : "bg-orange-50 text-orange-600 dark:bg-orange-900/20 dark:text-orange-400", border: resumo.saldo >= 0 ? "border-blue-100 dark:border-blue-900/30" : "border-orange-100 dark:border-orange-900/30" },
            { label: "Total de Lançamentos",  value: String(resumo.totalLancamentos), icon: ReceiptText, color: "bg-purple-50 text-purple-600 dark:bg-purple-900/20 dark:text-purple-400", border: "border-purple-100 dark:border-purple-900/30" },
          ].map((card, i) => (
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
        ...
        <div className="flex flex-col sm:flex-row flex-wrap items-start sm:items-center gap-3 p-4 border-b border-zinc-100 dark:border-zinc-800">
          ...
          <div className="relative">
            <input type="month" value={filtroMes} onChange={(e) => setFiltroMes(e.target.value)}
              className="pl-3 pr-8 py-1.5 text-xs rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 focus:outline-none focus:ring-2 focus:ring-green-500/30" />
          ...
          <div className="relative">
            <select value={filtroCategoria} onChange={(e) => setFiltroCategoria(e.target.value)}
              className="pl-3 pr-8 py-1.5 text-xs rounded-lg border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 focus:outline-none focus:ring-2 focus:ring-green-500/30 appearance-none">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16">
            ...
            <button onClick={abrirNovo} className="mt-4 text-xs text-green-600 dark:text-green-400 hover:underline font-medium">+ Criar primeiro lançamento</button>
          </div>
        ) : (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 16 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 16 }}
              ...
              <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-100 dark:border-zinc-800 sticky top-0 bg-white dark:bg-zinc-900 z-10">
                <h2 className="font-semibold text-zinc-900 dark:text-white">{editando ? "Editar Lançamento" : "Novo Lançamento"}</h2>
              ...
              <div className="px-6 py-5 space-y-4">
                ...
                <div>
                  ...
                  <input type="text" value={form.descricao} onChange={(e) => setForm((f) => ({ ...f, descricao: e.target.value }))}
                    placeholder="Ex: Comissão financiamento João Silva"
                    className="w-full px-3 py-2.5 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-green-500/30 placeholder-zinc-400" />
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <input type="date" value={form.data} onChange={(e) => setForm((f) => ({ ...f, data: e.target.value }))}
                      className="w-full px-3 py-2.5 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-green-500/30" />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <select value={form.categoria} onChange={(e) => setForm((f) => ({ ...f, categoria: e.target.value }))}
                      className="w-full px-3 py-2.5 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-green-500/30 appearance-none">
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {FORMAS_PAGAMENTO.map((fp) => (
                      <button key={fp.value} type="button"
                        ...
                        className={`py-2.5 rounded-xl text-xs font-semibold transition-all flex flex-col items-center gap-1 border ${form.formaPagamento === fp.value ? "bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 border-zinc-900 dark:border-white" : "bg-white dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 border-zinc-200 dark:border-zinc-700 hover:bg-zinc-50 dark:hover:bg-zinc-700"}`}>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <select value={form.parcelas} onChange={(e) => setForm((f) => ({ ...f, parcelas: e.target.value }))}
                        className="w-full px-3 py-2.5 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-purple-500/30 appearance-none">
                ...
                <div>
                  ...
                  <textarea value={form.observacao} onChange={(e) => setForm((f) => ({ ...f, observacao: e.target.value }))}
                    ...
                    className="w-full px-3 py-2.5 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-zinc-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-green-500/30 placeholder-zinc-400 resize-none" />
                </div>
              </div>
              ...
              <div className="flex gap-2 px-6 py-4 border-t border-zinc-100 dark:border-zinc-800 sticky bottom-0 bg-white dark:bg-zinc-900">
                ...
                <button onClick={salvar} disabled={salvando}
                  className={`flex-1 py-2.5 rounded-xl text-sm font-semibold text-white transition-colors ${form.tipo === "receita" ? "bg-green-500 hover:bg-green-600" : "bg-red-500 hover:bg-red-600"} disabled:opacity-60`}>
