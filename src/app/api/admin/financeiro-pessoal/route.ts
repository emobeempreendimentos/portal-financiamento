...
export async function GET(request: NextRequest) {
  try {
    ...
    const todos = await prisma.lancamentoPessoal.findMany({ orderBy: { data: "asc" } });

    const totalReceitas = todos.filter((l) => l.tipo === "receita").reduce((acc, l) => acc + l.valor, 0);
    const totalDespesas = todos.filter((l) => l.tipo === "despesa").reduce((acc, l) => acc + l.valor, 0);
